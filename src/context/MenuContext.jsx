...
//           const uniqueLocalCart = Array.from(new Map(localCart.map(item => [item._id, item])).values());
          
//           // Sync local cart with server
//           const response = await axios.post(`${url}/api/cart/sync`, { cartItems: uniqueLocalCart }, {
//             headers: { Authorization: `Bearer ${token}` }
//           });
...
//       if (response.data.success) {
//         setCartItems((prevCartItems) => {
//           const existingItemIndex = prevCartItems.findIndex(cartItem => cartItem._id === item._id);
//           const updatedCartItems = [...prevCartItems];
