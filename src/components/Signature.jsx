...
function Signature() {
  ...
  const currentItems = signatureItems.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(signatureItems.length / itemsPerPage);
    ...
    <div className="my-5 mx-2 sm:mx-5 lg:mx-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
        {currentItems.map((item) => (
          <Card key={item._id}>
            <Link to={`/details/${item._id}`} className="block relative">
              <img
      ...
      {/* Pagination Controls */}
      {signatureItems.length > itemsPerPage && (
